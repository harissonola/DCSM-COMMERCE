...
class ResetPasswordController extends AbstractController
{
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!$user) {
                return $this->handleErrorResponse(
                    $translator->trans('Aucun compte vérifié n\'est associé à cet email.'),
                    $request,
            ...
            try {
                ...
                $this->logger->error('Erreur lors de la génération du token : ' . $errorMessage);
            ...
            try {
                ...
                return $this->handleSuccessResponse(
                    $translator->trans('Un email de réinitialisation a été envoyé.'),
                    $request,
                    'app_check_email'
                );
                ...
                return $this->handleErrorResponse(
                    $translator->trans('Une erreur est survenue lors de l\'envoi de l\'email.'),
                    $request,
    ...
    public function reset(
        ...
        if (!$token) {
            return $this->handleErrorResponse(
                $translator->trans('Aucun token de réinitialisation trouvé.'),
                $request,
        ...
        if ($form->isSubmitted()) {
            if ($form->isValid()) {
                ...
                return $this->handleSuccessResponse(
                    $translator->trans('Votre mot de passe a été réinitialisé avec succès.'),
                    $request
                );
            } else {
                $errors = [];
     ...
     */
    private function handleSuccessResponse(string $message, Request $request, string $redirectRoute = null): Response
    {
        if ($request->isXmlHttpRequest()) {
            return new JsonResponse(['success' => true, 'message' => $message]);
        }
        return $this->redirectToRoute($redirectRoute ?? 'app_login');
    }
}